    ...
    if case_id not in times_log and case_id not in errors_log:
        try:
            if not os.path.exists(os.path.join(original_db_path, case_id, f"{case_id}_ncct.nii.gz")):
                shutil.copy(os.path.join(original_db_path, case_id, f"{case_id}_ncct_reg.nii.gz"), os.path.join(original_db_path, case_id, f"{case_id}_ncct.nii.gz"))
            start = time()
            ...
            preprocess_case(case_id=case_id,
                            ...
                            vessel_segmentation_path=os.path.join(original_db_path, case_id, f"{case_id}_extracranial_vessel_segmentation.nii.gz"),
